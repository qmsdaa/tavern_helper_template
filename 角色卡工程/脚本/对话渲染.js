 ...
 *   - 面板：同源内联 HTML/CSS/JS 设置面板（主题 / 气泡字号 / 旁白字号 / 行距 / 头像大小 / 头像自定义）
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { parseBubbleLine, renderBubblesInHtml, renderBubbleText, renderMessageHtml, parseSceneHeader, buildInjectionText, canonicalCastNames, openAvatarZoom, MOOD_GROUPS, MOOD_MAP, AVATAR_TABLE, KNOWN_NO_AVATAR, escapeHtml };
}
...
/** 具名角色规范名单：按头像 URL 去重别名（爱布拉娜·都柏林 / 爱布拉娜 只保留全名爱布拉娜·都柏林） */
function canonicalCastNames() {
  const names = [];
  const seenKey = new Set();
  for (const name of Object.keys(AVATAR_TABLE)) {
    const key = AVATAR_TABLE[name];
...
function buildInjectionText() {
  const castNames = canonicalCastNames().concat([...KNOWN_NO_AVATAR, ...loadExtraNames()]);
...
.cf-bub{display:flex;gap:14px;margin:16px 4px;align-items:flex-start;font-size:var(--cf-bubble-fs,17px);line-height:var(--cf-line-height,1.75)}
.cf-bub-avatar{flex:0 0 auto;box-sizing:border-box;width:var(--cf-avatar-size,48px);height:calc(var(--cf-avatar-size,48px) * 1.44);border-radius:8px;padding:2px;background:var(--cf-avatar-bg,#fdfaf4);
  box-shadow:0 0 0 2px var(--cf-ring,#b8a6ab),0 2px 8px rgba(120,90,100,.12);overflow:hidden;
  display:flex;align-items:center;justify-content:center;cursor:zoom-in;transition:transform .15s ease,box-shadow .15s ease}
.cf-bub-avatar:hover{transform:scale(1.04);box-shadow:0 0 0 2px var(--cf-ring,#b8a6ab),0 4px 14px rgba(120,90,100,.22)}
.cf-bub-avatar-img{width:100%;height:100%;object-fit:cover;border-radius:6px;display:block}
.cf-bub-avatar-fallback{font-size:calc(var(--cf-avatar-size,48px) * .42);color:var(--cf-accent-soft,#a5737f);font-weight:600;user-select:none;line-height:1}
.cf-bub-main{flex:1 1 auto;min-width:0;max-width:80%}
.cf-bub-plate{display:flex;align-items:baseline;gap:8px;margin:0 0 6px 4px}
.cf-bub-name{color:var(--cf-accent,#c05a72);font-weight:700;font-size:calc(var(--cf-bubble-fs,17px) * .85);letter-spacing:.5px}
.cf-bub-mood{font-size:calc(var(--cf-bubble-fs,17px) * .62);color:var(--cf-mood,#b8a6ab);border:1px solid currentColor;border-radius:10px;
  padding:1px 8px;line-height:1.4;opacity:.9;font-weight:500}
.cf-bub-inner-tag{font-size:calc(var(--cf-bubble-fs,17px) * .62);color:var(--cf-accent-soft,#a5737f);border:1px dashed #d9b8c0;border-radius:10px;padding:1px 8px;line-height:1.4;font-weight:500}
.cf-bub-bubble{position:relative;background:var(--cf-bub-bg,#fbf5ec);color:var(--cf-bub-text,#5b4a4f);
  ...
  padding:12px 16px;box-shadow:0 1px 4px rgba(140,100,110,.08);word-break:break-word;white-space:pre-wrap;
  font-size:var(--cf-bubble-fs,17px);line-height:var(--cf-line-height,1.75)}
...
.cf-scene-banner{text-align:center;margin:18px 8px 14px;padding:8px 4px;letter-spacing:2px;
  border-top:1px solid rgba(192,90,114,.35);border-bottom:1px solid rgba(192,90,114,.35)}
.cf-scene-time{display:block;font-size:calc(var(--cf-bubble-fs,17px) * .85);font-weight:600;color:var(--cf-accent-bright,#d97a92)}
.cf-scene-meta{display:block;font-size:calc(var(--cf-bubble-fs,17px) * .7);opacity:.75;margin-top:3px;letter-spacing:1px}
...
#chat .cf-bub-host{background:var(--cf-page-bg,#f6efdc);color:var(--cf-page-text,#5b4a4f);
  border:1px solid var(--cf-page-border,#e7dbc0);border-radius:12px;padding:14px 18px;margin:10px 0;
  box-shadow:0 1px 6px var(--cf-page-shadow,rgba(140,110,80,.10))}
#chat .cf-bub-host>p:not(:has(img)){text-indent:2em;text-align:justify;line-height:var(--cf-line-height,1.8);margin:8px 2px;font-size:var(--cf-narrative-fs,15.5px);opacity:.95}
...
/* 面板（同源内联版，无 iframe/CDN 依赖，关闭后随 #cf-bubble-panel-mask 一起销毁） */
.cf-panel-mask{position:fixed;inset:0;background:rgba(58,38,48,.40);z-index:99990}
.cf-panel{position:fixed;top:5vh;left:50%;transform:translateX(-50%);width:min(620px,94vw);max-height:88vh;overflow-y:auto;
  background:#fffdf9;color:#5b4a4f;border:1px solid #ecdde2;border-radius:16px;z-index:99991;
  box-shadow:0 12px 48px rgba(90,60,70,.30);padding:0;font-size:14px;line-height:1.6;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","PingFang SC","Hiragino Sans GB","Microsoft YaHei",sans-serif}
.cf-panel *{box-sizing:border-box}
.cf-panel .cf-panel-head{position:relative;background:linear-gradient(135deg,#fdf3f5,#fbf6f2);border-bottom:1px solid #f0e2e6;padding:16px 22px 14px;border-radius:16px 16px 0 0}
.cf-panel h2{margin:0 0 4px;font-size:18px;color:#c05a72;letter-spacing:2px;font-weight:700}
.cf-panel .cf-panel-subtitle{font-size:12px;color:#a08a90;margin:0}
.cf-panel .cf-panel-body{padding:6px 22px 20px}
.cf-panel .cf-sec{background:#fdf8f3;border:1px solid #f3e6e0;border-radius:12px;padding:12px 16px;margin:14px 0}
.cf-panel h3{margin:0 0 10px;font-size:12px;color:#a5737f;letter-spacing:2px;font-weight:700;display:flex;align-items:center;gap:7px}
.cf-panel h3::before{content:"";width:3px;height:14px;background:#e87a90;border-radius:2px;flex:0 0 auto}
.cf-panel .cf-row{display:flex;align-items:center;gap:12px;margin:11px 0}
.cf-panel .cf-row label{flex:0 0 5em;font-size:13px;color:#6b5a58}
.cf-panel .cf-row input[type=range]{flex:1;min-width:0;accent-color:#e87a90;cursor:pointer}
.cf-panel .cf-row .cf-val{flex:0 0 3.6em;text-align:right;font-size:12px;color:#c05a72;font-weight:600}
.cf-panel .cf-theme-row{display:flex;gap:8px;margin:4px 0 2px}
.cf-panel .cf-theme-row button{flex:1;padding:9px 0;border:1px solid #ecdde2;background:#fff;color:#8a6f76;border-radius:10px;cursor:pointer;font-size:13px;font-weight:500;transition:all .15s}
.cf-panel .cf-theme-row button:hover{border-color:#e87a90;color:#c05a72;background:#fff6f8}
.cf-panel .cf-theme-active{background:linear-gradient(135deg,#e87a90,#d96b80) !important;color:#fff !important;border-color:#e87a90 !important;box-shadow:0 2px 8px rgba(232,122,144,.35)}
.cf-panel .cf-panel-enabled{display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer;padding:2px 0 4px;color:#5b4a4f}
.cf-panel .cf-panel-enabled input{accent-color:#e87a90;width:16px;height:16px;cursor:pointer}
.cf-panel .cf-close{position:absolute;top:14px;right:16px;border:none;background:rgba(255,255,255,.7);font-size:18px;color:#a5737f;cursor:pointer;line-height:1;padding:4px 9px;border-radius:8px;transition:all .15s}
.cf-panel .cf-close:hover{color:#fff;background:#e87a90}
.cf-panel .cf-add-row{display:flex;gap:8px;margin:4px 0 2px}
.cf-panel .cf-add-row input[type=text]{flex:1;min-width:0;border:1px solid #e3d3d8;border-radius:10px;padding:9px 12px;background:#fff;color:#5b4a4f;font-size:13px;outline:none;box-shadow:inset 0 1px 3px rgba(140,100,110,.06);transition:border-color .15s,box-shadow .15s}
.cf-panel .cf-add-row input[type=text]:focus{border-color:#e87a90;box-shadow:0 0 0 3px rgba(232,122,144,.12)}
.cf-panel .cf-add-row button{padding:9px 18px;border:none;background:linear-gradient(135deg,#e87a90,#d96b80);color:#fff;border-radius:10px;cursor:pointer;font-size:13px;font-weight:600;white-space:nowrap;box-shadow:0 2px 8px rgba(232,122,144,.28);transition:filter .15s}
.cf-panel .cf-add-row button:hover{filter:brightness(1.06)}
.cf-panel .cf-avatar-list{max-height:40vh;overflow-y:auto;padding:2px 4px;margin:2px -4px 0;scrollbar-width:thin}
.cf-panel .cf-avatar-row{display:flex;align-items:center;gap:10px;padding:7px 6px;border-bottom:1px solid #f6ecee}
.cf-panel .cf-avatar-row:last-child{border-bottom:none}
.cf-panel .cf-avatar-row img{width:40px;height:56px;border-radius:6px;object-fit:cover;background:#f3ece2;flex:0 0 auto;cursor:zoom-in;box-shadow:0 1px 3px rgba(120,90,100,.15)}
.cf-panel .cf-avatar-row .cf-name{flex:0 0 6.5em;font-weight:600;font-size:13px;color:#5b4a4f;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.cf-panel .cf-avatar-row input[type=text]{flex:1;min-width:0;border:1px solid #e3d3d8;border-radius:8px;padding:6px 10px;background:#fff;color:#5b4a4f;font-size:12px;outline:none}
.cf-panel .cf-avatar-row input[type=text]:focus{border-color:#e87a90}
.cf-panel .cf-avatar-row button{padding:6px 12px;border:1px solid #e87a90;background:#fff6f8;color:#c05a72;border-radius:8px;cursor:pointer;font-size:12px;transition:all .15s;white-space:nowrap}
.cf-panel .cf-avatar-row button:hover{background:#e87a90;color:#fff}
.cf-panel .cf-avatar-row button.cf-reset{border-color:#e3d3d8;color:#a08a90;background:#fff}
.cf-panel .cf-avatar-row button.cf-reset:hover{background:#f3ece2;color:#5b4a4f;border-color:#e87a90}
.cf-panel .cf-avatar-row button.cf-del{border-color:#e5b6b6;color:#b06060;background:#fff}
.cf-panel .cf-avatar-row button.cf-del:hover{background:#d9534f;color:#fff;border-color:#d9534f}
.cf-panel .cf-panel-note{font-size:11.5px;color:#a08a90;margin-top:14px;line-height:1.7}
.cf-panel .cf-stats{font-size:11.5px;color:#a08a90;margin-top:6px;text-align:right}
`;
...
const LS_EXTRA_NAMES_KEY = 'cf_bubble_extra_names_v1';
const LS_UPLOAD_KEY = 'cf_bubble_uploaded_avatars_v1';

function loadConfig() {
  const defaults = { enabled: true, theme: 'parchment', bubbleFontSize: 17, narrativeFontSize: 15.5, lineHeight: 1.75, avatarSize: 48 };
  try {
...
/* 玩家自建角色名单（面板「新增角色」写入；node 无头测试下无 localStorage，返回空数组） */
function loadExtraNames() {
  try {
    if (typeof localStorage === 'undefined') return [];
    const arr = JSON.parse(localStorage.getItem(LS_EXTRA_NAMES_KEY) || '[]');
...
function saveExtraNames(arr) {
  try {
    if (typeof localStorage === 'undefined') return;
    localStorage.setItem(LS_EXTRA_NAMES_KEY, JSON.stringify(arr));
  } catch (_) {}
}

/* 本地上传头像：以 Base64 data URL 存 localStorage（同步、跨刷新持久、无 objectURL 生命周期问题） */
...
function saveUploadedAvatars(map) {
  try {
    if (typeof localStorage === 'undefined') return;
    localStorage.setItem(LS_UPLOAD_KEY, JSON.stringify(map));
  } catch (_) {}
}

// ████████████████████████████████████████████████████████████
// █  Part 4: 头像解析（自定义 URL > 本地上传 > 预置表 > 占位） █
// ████████████████████████████████████████████████████████████

function resolveAvatarSync(name) {
  const customUrls = loadCustomUrls();
  if (customUrls[name]) return customUrls[name];
  const uploaded = loadUploadedAvatars();
  if (uploaded[name]) return uploaded[name];
  return AVATAR_TABLE[name] || null;
}

/** 异步版（与同步版逻辑一致，保留给可能的外部调用） */
async function resolveAvatar(name) {
  return resolveAvatarSync(name);
}
...
function applyConfig(doc) {
  ...
  chat.style.setProperty('--cf-avatar-size', `${cfg.avatarSize}px`);
}
...
// ████████████████████████████████████████████████████████████
// █  Part 7: 设置与头像自定义面板（同源内联版，无 iframe/CDN）█
// ████████████████████████████████████████████████████████████
...
/* 收集当前已渲染/缓存中出现的角色名（跟 openPanel 同步） */
function collectSeenNames(doc) {
  const seen = new Set();
  const chat = doc.getElementById('chat');
  if (!chat) return seen;
  for (const elem of chat.querySelectorAll('.cf-bub[data-name]')) {
    const n = elem.getAttribute('data-name');
    if (n) seen.add(n);
  }
  for (const text of doc.querySelectorAll('#chat .mes_text, #chat .mes-text')) {
    const original = originalHtmlCache.get(text) || text.innerHTML || '';
...
/* 应用设置变更（输入栏滑块、主题、头像 URL/上传）：女巫改动以后立刻生效 */
function applyConfigChange(doc, patch) {
  const next = { ...loadConfig(), ...patch };
  saveConfig(next);
  applyConfig(doc);
  applyTheme(doc);
  if (next.enabled) renderAllMessages(doc, true);
}

/* 面板角色名去重：预置别名按头像 URL 折叠（爱布拉娜·都柏林 / 爱布拉娜 只留全名），其余按名字去重 */
function collectPanelNames(doc) {
  const seen = collectSeenNames(doc);
  const names = [];
  const seenKey = new Set();
  const add = (name) => {
    if (!name) return;
    const key = AVATAR_TABLE[name] || name;
    if (seenKey.has(key)) return;
    seenKey.add(key);
    names.push(name);
  };
  Object.keys(AVATAR_TABLE).forEach(add);
  KNOWN_NO_AVATAR.forEach(add);
  loadExtraNames().forEach(add);
  seen.forEach(add);
  return names;
}

/* 单行头像 HTML；extra=true 追加「删除」按钮（玩家自建角色可移除） */
function makeAvatarRowHtml(name, customUrls, extra) {
  const preset = AVATAR_TABLE[name] || '';
  const custom = customUrls[name] || '';
  const uploaded = loadUploadedAvatars()[name] || '';
  const initial = custom || uploaded || preset || '';
  const uploadMark = uploaded ? '（已上传本地图）' : '';
  const placeholder = preset ? '预置头像（粘贴 URL 可覆盖）' : '粘贴图片 URL 或点上传';
  const delBtn = extra ? '<button data-act="del" class="cf-del">删除</button>' : '';
  return `<div class="cf-avatar-row" data-name="${escapeHtml(name)}">
    <img src="${escapeHtml(initial || 'data:image/gif;base64,R0lGODlhAQABAAAAACw=')}" alt="" />
    <span class="cf-name" title="${escapeHtml(name)}">${escapeHtml(name)}</span>
    <input type="text" placeholder="${escapeHtml(placeholder)}" value="${escapeHtml(custom)}" />
    <button data-act="upload">上传</button>
    <button data-act="reset" class="cf-reset">重置</button>
    ${delBtn}
    ${uploadMark}
  </div>`;
}

/* 为单行头像绑定事件：URL / 上传 / 重置 / 删除（自建）/ 点击放大 */
function wireAvatarRow(row, doc) {
  ...
  input.addEventListener('change', () => {
    ...
    applyConfigChange(doc, { _avatarTick: Date.now() });
  });
  row.querySelector('[data-act=upload]')?.addEventListener('click', () => {
    ...
    picker.onchange = () => {
      ...
      if (file.size > 3 * 1024 * 1024) {
        alert('图片过大（超过 3MB），请选择更小的图片');
        return;
      }
      const reader = new FileReader();
      reader.onload = () => {
        const uploaded = loadUploadedAvatars();
        uploaded[name] = reader.result;
        saveUploadedAvatars(uploaded);
  ...
  row.querySelector('[data-act=reset]')?.addEventListener('click', () => {
    ...
    saveUploadedAvatars(uploaded);
    input.value = '';
    img.src = AVATAR_TABLE[name] || 'data:image/gif;base64,R0lGODlhAQABAAAAACw=';
    applyConfigChange(doc, { _avatarTick: Date.now() });
  });
  row.querySelector('[data-act=del]')?.addEventListener('click', () => {
    ...
    delete uploaded[name];
    saveUploadedAvatars(uploaded);
    saveExtraNames(loadExtraNames().filter(n => n !== name));
    row.remove();
    applyInjection();
    applyConfigChange(doc, { _avatarTick: Date.now() });
  });
  img.addEventListener('click', () => {
    openAvatarZoom(doc, img.src, name);
  });
}
...
function openPanel(doc) {
  ...
  const customUrls = loadCustomUrls();
  const extraSet = new Set(loadExtraNames());
  const names = collectPanelNames(doc);
  ...
  const rows = names.map(name => makeAvatarRowHtml(name, customUrls, extraSet.has(name))).join('');

  panel.innerHTML = `
    <div class="cf-panel-head">
      <button class="cf-close" title="关闭">×</button>
      <h2>🌸 对话气泡 · Counterfeit</h2>
      <p class="cf-panel-subtitle">主题、字号、头像设置均立即生效并保存在本地浏览器</p>
      <label class="cf-panel-enabled">
        <input type="checkbox" id="cf-enabled" ${config.enabled ? 'checked' : ''} />
        启用对话气泡（关闭后不再注入格式规则，也不渲染）
      </label>
    </div>
    <div class="cf-panel-body">
      <div class="cf-sec">
        <h3>主题</h3>
        <div class="cf-theme-row" id="cf-theme-row">
          <button data-theme="parchment" class="${config.theme === 'parchment' ? 'cf-theme-active' : ''}">羊皮纸</button>
      ...
      <div class="cf-sec">
        ...
        <div class="cf-row">
          <label>气泡字号</label>
          <input type="range" id="cf-bubble-fs" min="12" max="22" step="0.5" value="${config.bubbleFontSize}" />
          <span class="cf-val" id="cf-bubble-fs-val">${config.bubbleFontSize}px</span>
        </div>
        <div class="cf-row">
          <label>旁白字号</label>
          <input type="range" id="cf-narrative-fs" min="12" max="22" step="0.5" value="${config.narrativeFontSize}" />
          <span class="cf-val" id="cf-narrative-fs-val">${config.narrativeFontSize}px</span>
        </div>
        <div class="cf-row">
          <label>行距</label>
          <input type="range" id="cf-line-height" min="1.4" max="2.0" step="0.05" value="${config.lineHeight}" />
          <span class="cf-val" id="cf-line-height-val">${config.lineHeight}</span>
        </div>
        <div class="cf-row">
          <label>头像大小</label>
          <input type="range" id="cf-avatar-size" min="36" max="72" step="2" value="${config.avatarSize}" />
          <span class="cf-val" id="cf-avatar-size-val">${config.avatarSize}px</span>
        </div>
      </div>
      ...
      <div class="cf-sec">
        ...
        <div class="cf-add-row">
          <input type="text" id="cf-add-name" placeholder="输入角色全名（AI 输出时使用的名字）" />
          <button id="cf-add-btn">添加角色</button>
        </div>
      </div>

      <div class="cf-sec">
        <h3>头像自定义（点击预览可放大）</h3>
        <div class="cf-avatar-list">${rows}</div>
      </div>

      <p class="cf-panel-note">
        粘贴 URL 立即覆盖预置头像；「上传」把图片存进浏览器 IndexedDB（不上传服务器）；「重置」清除该角色的自定义 URL 与上传图。<br>
        新增角色会加入格式注入名单，模型按其全名输出 @bubble 行即可渲染出对应头像；自建角色可点「删除」移除。<br>
        头像描边与徽记颜色由情绪字段决定（喜悦金／愤怒红／悲伤蓝／紧张赭／平和绿／害羞青／嫌弃紫／爱恋粉）。
      </p>
    </div>`;
  ...
  panel.querySelector('#cf-enabled').addEventListener('change', (ev) => {
    applyConfigChange(doc, { enabled: ev.target.checked });
    if (ev.target.checked) {
      applyInjection();
    } else {
      let handle = injectionHandle;
      if (handle) { try { handle.uninject(); } catch (_) {} injectionHandle = null; }
    }
  });
  ...
  panel.querySelectorAll('#cf-theme-row [data-theme]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      applyConfigChange(doc, { theme });
      panel.querySelectorAll('#cf-theme-row [data-theme]').forEach(b => {
        b.classList.toggle('cf-theme-active', b === btn);
      });
    });
  });

  /* 字体 / 头像大小滑块 */
  const wire = (id, key, valId, fmt = v => `${v}px`) => {
    const input = panel.querySelector(id);
    const label = panel.querySelector(valId);
    if (!input || !label) return;
    input.addEventListener('input', () => {
      const v = key === 'lineHeight' ? Number(input.value) : Number(input.value);
      label.textContent = fmt(v);
      applyConfigChange(doc, { [key]: v });
    });
  };
  wire('#cf-bubble-fs', 'bubbleFontSize', '#cf-bubble-fs-val');
  wire('#cf-narrative-fs', 'narrativeFontSize', '#cf-narrative-fs-val');
  wire('#cf-line-height', 'lineHeight', '#cf-line-height-val', v => String(v));
  wire('#cf-avatar-size', 'avatarSize', '#cf-avatar-size-val');

  /* 头像行：URL / 上传 / 重置 / 删除 / 点击预览放大 */
  panel.querySelectorAll('.cf-avatar-row').forEach(row => wireAvatarRow(row, doc));

  /* 新增角色：写入名单 → 追加一行 → 重新注入格式名单 */
  const addInput = panel.querySelector('#cf-add-name');
  const addBtn = panel.querySelector('#cf-add-btn');
  const addCharacter = () => {
    const name = addInput.value.trim();
    if (!name) { alert('请输入角色名'); return; }
    if (AVATAR_TABLE[name] || KNOWN_NO_AVATAR.includes(name) || loadExtraNames().includes(name)) {
      alert('该角色已存在，无需重复添加');
      return;
    }
    const extras = loadExtraNames();
    extras.push(name);
    saveExtraNames(extras);
    const wrapper = doc.createElement('div');
    wrapper.innerHTML = makeAvatarRowHtml(name, loadCustomUrls(), true);
    const rowEl = wrapper.firstElementChild;
    panel.querySelector('.cf-avatar-list').appendChild(rowEl);
    wireAvatarRow(rowEl, doc);
    addInput.value = '';
    applyInjection();
  };
