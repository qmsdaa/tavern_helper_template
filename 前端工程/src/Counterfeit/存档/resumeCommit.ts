import { ACU_CHAT_ID_SUFFIX, toShujukuSheetData } from './acuTables';
...
export interface ResumeCommitDeps {
  ...
  /**
   * shujuku 插件运行时导入通道（best-effort，失败不阻断迁移）。
   * 调用方实现：把 sheetData 序列化后走 window.top.AutoCardUpdaterAPI.importTableAsJson；
   * 返回 true 表示插件已接管运行时表格。
   */
  importTables?: (sheetData: Record<string, unknown>) => Promise<boolean>;
}
...
export async function commitPortableResume(result: MigrationResult, deps: ResumeCommitDeps): Promise<void> {
  ...
  const sheetData = toShujukuSheetData(tableSheets);
  ...
  try {
    ...
    if (canMigrateTables) {
      ...
      // 插件运行时导入（best-effort）：shujuku 原生导入通道会按当前存储模式持久化并通知刷新；
      // 失败不阻断迁移（chat_metadata 已兜底，native 模式重载可恢复）。
      if (sheetData && typeof deps.importTables === 'function') {
        try {
          const imported = await deps.importTables(sheetData);
          if (imported) {
            console.info('[Counterfeit·迁移] 数据库表格已通过 shujuku 插件导入');
          } else {
            console.warn('[Counterfeit·迁移] shujuku 插件导入未生效，由 chat_metadata 兜底');
          }
        } catch (error) {
          console.warn('[Counterfeit·迁移] shujuku 插件导入失败，由 chat_metadata 兜底', error);
        }
      }
    }
  } catch (error) {
